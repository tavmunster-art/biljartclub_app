...
def init_db():
    ...
    tournament_columns = {row["name"] for row in conn.execute("PRAGMA table_info(tournaments)")}
    if "format" not in tournament_columns:
        conn.execute("ALTER TABLE tournaments ADD COLUMN format TEXT NOT NULL DEFAULT 'groups'")
